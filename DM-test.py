...
for bb_tkr in oot.bb_tkr:
    ...
    exposure = getexposure(type_of_trader = models1.loc[model_id1,'cot_type'], norm = models1.loc[model_id1,'cot_norm'], bb_tkr = bb_tkr)
    exposure.columns = exposure.columns.droplevel(0)
    exposure['diffs'] = exposure.net_specs.diff()
    
    forecast1 = pd.read_sql_query(f"SELECT * FROM cftc.forecast WHERE model_id = {model_id1}",engine1,index_col = 'px_date')
    forecast1.columns = ['model_id','fcast1']
    forecast2 = pd.read_sql_query(f"SELECT * FROM cftc.forecast WHERE model_id = {model_id2}",engine1,index_col = 'px_date')
