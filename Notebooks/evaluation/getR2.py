...
#For overview
model_types = pd.read_sql_query("SELECT * from cftc.model_type_desc where model_type_id IN (82,76,95,100,137,93)", engine1).set_index('model_type_id')
# models = pd.read_sql_query(f" Select * from cftc.model_desc where model_type_id = 137", engine1).set_index('model_id')
# models.head()
