...
def empirical_vals2(model1_type_id, model2_type_id, bb_tkr):
    ...
    model_types = pd.read_sql_query(
        f"SELECT * from cftc.model_type_desc where model_type_id IN ({model1_type_id},{model2_type_id})",
        engine1).set_index('model_type_id')
    ...
    if list(model_types.cot_type)[0] != list(model_types.cot_type)[1]:
        ...
        empirical_vals = getexposure(type_of_trader=model_types.loc[model1_type_id, 'cot_type'],
                                     norm=model_types.loc[model1_type_id, 'cot_norm'], bb_tkr=bb_tkr)
...
def Zarnowitz2Models(model1_type_id, model2_type_id):
    ...
    result = pd.DataFrame(index=bb_tkrs,
                          columns=['m1', 'm2', 'beta_m1', 'beta_m2', 'tstat_m1', 'pval_m1', 'tstat_m2', 'pval_m2',
                                   'rsquared', 'nobs'])
    ...
    for bb_tkr in bb_tkrs:
        ...
        df = pd.merge(fcast_m1[['px_date', 'qty']], fcast_m2[['px_date', 'qty']], how='inner', on='px_date',
                      suffixes=('_m1', '_m2'))
